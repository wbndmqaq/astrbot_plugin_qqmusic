...
*   **🔗 智能解析放行卡片发送**：卡片与链接解析成功后，根据用户配置完整放行音乐卡片、语音与群文件投递（`skipTextInfo: True` 跳过冗余纯文本），同时打上 `_detailFetched` 标记，避免二次重复请求 `songDetail`。
*   **📝 歌词支持序号/链接/关键词全格式查词**：
    *   支持 `#qqm歌词 1~99` 直接提取当前点歌/解析会话列表第 N 首歌曲的歌词，彻底杜绝将数字序号误当关键词搜索导致的失败；
    *   优化关键词查词时的异常捕获与类型守卫，避免搜索或接口失败时静默无响应；
    *   保留多页歌词分页完整渲染与纯文本兜底。
*   **🎧 推荐歌单一键展开（`#qqm推荐听 序号`）**：
    *   补齐 `#qqm推荐听 序号` 闭环支持，可直接打开热门推荐列表中第 N 个歌单并展示歌曲列表；
    *   支持后续回复 `#qqm听序号` 播放单曲或 `#qqm听所有` 连播整张歌单。
*   **🛡 会话类型防御与精准引导**：
    *   在榜单分类（`topCategory`）或推荐歌单（`recommend`）会话中误执行 `#qqm歌词 序号`、`#qqmMV 序号` 或 `#qqm听 序号` 时，给出明确引导（提示先查看榜单/歌单歌曲），杜绝报无意义错误或提取空数据。
*   **🌐 apiBase 容错与自动补全**：
    *   `api.py` 新增 `normalize_api_base` 规整 API 地址（自动补齐 `http://` 协议头、全角冒号 `：` 转半角 `:`、清理 BOM 与首尾引号/空格）；
    *   `#qqm api <地址>` 主人指令支持参数自动规整与协议补全，并在修改成功后返回脱敏地址反馈（如 `http://***.com:3300`）。
*   **⚡ 全库全面异步化与非阻塞 IO**：
    *   音频大文件下载（`download_audio`）、卡片渲染 PNG 缓存落盘（`_render_card`）、登录二维码（`_save_qr_image`）全面接入 `asyncio.to_thread(_write_bytes, ...)` 异步写入，彻底消除主事件循环因磁盘 IO 导致的卡顿；
    *   修复扫码登录轮询（`_start_poll` / `_start_webqr_poll`）与临时文件延迟清理中的事件循环引用及作用域问题，全面提升高并发稳定性。
*   **📖 帮助卡片与文档同步**：同步更新 `README.md` 与内置帮助卡片（`build_help_card_data`）中 `#qqm歌词` 与 `#qqm推荐听` 的用法说明。

---
## [v1.7.5] - 语音兼容性修复 + 歌词分页 + 分享卡/专辑/MV 体验修复

*   **🔒 移除系统级自动安装**（插件市场安全审查整改）：删除 render.py 中的 apt 源改写、`apt-get update`、`pip install playwright`、Chromium 二进制自动下载与 `install-deps` 自动执行——插件不再执行任何未经用户确认的系统级安装/网络下载，代码中不再存在任何 subprocess 安装调用。
...
*   **📖 README 教程化**：「卡片渲染」章节扩写为四步完整教程（装包 → 下载内核 → 系统运行库 / 可选换阿里源 → 重载插件），并声明插件绝不自动执行任何系统级安装。
*   **▶ 新增 `#qqm听所有`**: 对当前会话歌曲列表（点歌/专辑曲目/歌单展开/电台/日推/收藏等）依次发送全部歌曲的语音+音频文件（上限 30 首，防误触发刷屏）；单曲失败不中断，结束汇报成功/失败数。
*   **💿 专辑选择链路对齐反馈**：`#qqm专辑 关键词` → 带序号前 10 条候选 → `#qqm听N` 展开该专辑曲目 → `#qqm听N` 播放 / `#qqm听所有` 连播整张专辑；列表卡片与提示语同步更新。
*   **📖 帮助卡片**：点歌播放分区新增「连播列表」说明。
...
*   **📝 歌词完整显示**: 移除 36 行截断，超长歌词自动按每页 36 行分页成多张卡片发送。
*   **📋 分享卡片解析修复**: 解析入口从 `@filter.regex` 改为全量事件监听——OneBot `json` 段（音乐分享卡片）不会写入 `message_str`，regex 过滤器永远匹配不到，导致群里发卡片 bot 无反应；同时 `_collect_message_text` 显式读取 Json 组件的 ark 数据。
*   **💿 专辑列表可选**: `#qqm专辑 关键词` 不再直接跳第一个结果，先出专辑候选列表（含曲数/发行时间），`#qqm听N` 展开曲目再选歌播放；唯一命中时保持直出，搜索候选扩到 8 个缓解新专辑搜不准。
*   **🎬 MV 搜索/分类卡片渲染**: MV 列表复用 list 卡片模板（封面/时长/序号），文本兜底保留；MV 发送失败回退链接的文案去掉「视频发送失败」措辞。
*   **📻 电台卡片渲染**: `#qqm电台` 与日推/收藏一致使用列表卡片。
*   **❤️ 收藏扩容**: `#qqm收藏` 请求 100 首（接口不支持时自动降级 30）。
